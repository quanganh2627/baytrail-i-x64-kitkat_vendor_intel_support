...
make_softap() {
    echo "  Making wl1283-ap SoftAP driver module"
    local SOFTAP_SRC_DIR=$TOP/hardware/ti/wlan/wl1283-ap/
    local MODULE_DEST_TMP=${PRODUCT_OUT}/kernel_modules
...
main() {
    ...
    do
        echo >&3 
        echo >&3 "Building SoftAP kernel module for $custom_board"
        echo >&3 "-----------------------------------------------"
        init_variables "$custom_board"
        make_softap ${custom_board} 
        exit_on_error $?
    done
    exit 0
}
